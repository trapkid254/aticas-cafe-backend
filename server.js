...
app.get("/api/admin/profile", authenticateAdmin, (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Failed to load admin profile" });
  }
});
...
app.get("/butchery-admin/login", (req, res) => {
  res.sendFile(
    path.join(__dirname, "../frontend/butchery-admin/butcheryadmin-login.html"),
  );
});
...
app.get("/butchery-admin/dashboard", (req, res) => {
  res.sendFile(
    path.join(__dirname, "../frontend/butchery-admin/butcheryadmins.html"),
  );
});
...
app.get("/butchery-admin/bookings", (req, res) => {
  res.sendFile(
    path.join(__dirname, "../frontend/butchery-admin/butcherybookings.html"),
  );
});
...
app.get("/butchery-admin/meat-management", (req, res) => {
  res.sendFile(
    path.join(__dirname, "../frontend/butchery-admin/meat-management.html"),
  );
});
...
app.get("/butchery-admin/payments", (req, res) => {
  res.sendFile(
    path.join(__dirname, "../frontend/butchery-admin/payments.html"),
  );
});
...
app.get("/butchery-admin/place-order", (req, res) => {
  res.sendFile(
    path.join(__dirname, "../frontend/butchery-admin/place-order.html"),
  );
});
...
app.delete("/api/admins/:id", authenticateAdmin, async (req, res) => {
  try {
    ...
    if (admin.employmentNumber === "AC001" || admin.role === "superadmin") {
      return res
        .status(403)
        .json({ success: false, error: "Cannot delete the super admin." });
    }
    ...
    orders = orders
      ...
      .map((order) => {
        ...
        o.total = (o.items || []).reduce((sum, item) => {
          const unitPrice =
            item.selectedSize && typeof item.selectedSize.price === "number"
...
app.get("/api/orders/:id", authenticateAdmin, async (req, res) => {
  try {
    ...
    if (!hasAccess) {
      return res
        .status(403)
        .json({ error: "Unauthorized to access this order" });
    }
...
app.post("/api/orders", async (req, res) => {
  ...
  try {
    ...
    for (const item of req.body.items) {
      if (
        !item.itemType ||
        !["Menu", "MealOfDay", "Meat"].includes(item.itemType)
      ) {
        return res
          .status(400)
          .json({
            success: false,
            error: "Invalid itemType for one or more items.",
          });
      }
      ...
      if (!found) {
        return res
          ...
          .json({
            success: false,
            error: `Invalid menuItem for itemType ${item.itemType}.`,
          });
      }
      ...
      if (item.selectedSize) {
        if (item.itemType === "Menu" && found.priceOptions) {
          const validSize = found.priceOptions.some(
            (option) =>
              option.size === item.selectedSize.size &&
    ...
    const itemsWithAdminType = await Promise.all(
      req.body.items.map(async (item) => {
        ...
        if (item.itemType === "Menu" || item.itemType === "Meat") {
          ...
          return {
            ...
            adminType:
              menuItem?.adminType ||
              (item.itemType === "Meat" ? "butchery" : "cafeteria"),
          };
        }
...
app.put("/api/orders/:id", authenticateAdmin, async (req, res) => {
  try {
    ...
    if (typeof viewedByAdmin === "boolean")
      updateDoc.viewedByAdmin = viewedByAdmin;
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to add menu item" });
    }
  },
);
...
app.put("/api/menu/:id", authenticateAdmin, async (req, res) => {
  try {
    ...
    if (existingItem.adminType !== req.admin?.adminType) {
      return res
        .status(403)
        .json({ error: "Unauthorized to modify this menu item" });
    }
    ...
    if (req.body.priceOptions) {
      for (const option of req.body.priceOptions) {
        if (!option.size || !option.price) {
          return res
            .status(400)
            .json({ error: "Each price option must have both size and price" });
        }
      }
    }
...
app.delete("/api/menu/:id", authenticateAdmin, async (req, res) => {
  try {
    ...
    if (existingItem.adminType !== req.admin?.adminType) {
      return res
        .status(403)
        .json({ error: "Unauthorized to delete this menu item" });
    }
...
app.get("/api/dashboard/stats", authenticateAdmin, async (req, res) => {
  try {
    ...
    const recentOrders = orders.slice(0, 5).map((o) => o); // already populated
...
app.post("/api/mpesa/payment", async (req, res) => {
  return res.status(410).json({
    error:
      "M-Pesa STK is temporarily disabled. Please pay manually to Till 634536.",
  });
});
...
app.get(
  "/api/orders/by-merchant-request/:merchantRequestId",
  async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ success: false, error: "Failed to lookup order" });
    }
  },
);
...
app.post("/api/orders/cancel", async (req, res) => {
  try {
    ...
    if (!merchantRequestId) {
      return res
        .status(400)
        .json({ success: false, error: "merchantRequestId is required" });
    }
    const order = await Order.findOne({ merchantRequestId });
    if (!order) {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Failed to cancel order" });
  }
});
  ...
  try {
    ...
    if (adminType !== "butchery" && role !== "superadmin") {
      return res
        ...
        .json({
          success: false,
          error: "Unauthorized: butchery admin required",
          detected: { adminType, role },
        });
    }
    ...
    if (!name || typeof price !== "number" || !image) {
      return res
        ...
        .json({
          success: false,
          error: "Name, numeric price, and image are required.",
        });
    }
    ...
    return res
      .status(500)
      .json({ success: false, error: "Failed to add meat" });
  }
});
...
app.put("/api/meats/:id", authenticateAdmin, async (req, res) => {
  try {
    ...
    if (!existing)
      return res
        .status(404)
        .json({ success: false, error: "Meat item not found" });
    const role = req.admin?.role || "admin";
    if (existing.adminType !== "butchery" && role !== "superadmin") {
      return res
        ...
        .json({
          success: false,
          error: "Unauthorized to modify this item",
          detected: { adminType: req.admin?.adminType, role },
        });
    }
    ...
    const updated = await Menu.findByIdAndUpdate(req.params.id, update, {
      new: true,
    });
    if (!updated)
      return res
    ...
    return res
      .status(500)
      .json({ success: false, error: "Failed to update meat" });
  }
});
...
app.delete("/api/meats/:id", authenticateAdmin, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Failed to delete meat" });
  }
});
...
app.post("/api/meals", authenticateAdmin, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to add meal of the day" });
  }
});
...
app.put("/api/meals/:id", authenticateAdmin, async (req, res) => {
  try {
    ...
    if (existingMeal.adminType !== req.admin?.adminType) {
      return res
        .status(403)
        .json({ error: "Unauthorized to modify this meal" });
    }
    ...
    const updatedMeal = await MealOfDay.findByIdAndUpdate(
      req.params.id,
      updateData,
      { new: true },
    );
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to update meal of the day" });
  }
});
...
app.delete("/api/meals/:id", authenticateAdmin, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to delete meal of the day" });
  }
});
...
app.get("/api/cart/:userId", async (req, res) => {
  try {
    ...
    for (let item of cart.items) {
      if (item.itemType === "Menu" || item.itemType === "Meat") {
        item.menuItem = await Menu.findById(item.menuItem).select(
          "name price image priceOptions category adminType",
        );
        ...
        item.menuItem = await MealOfDay.findById(item.menuItem).select(
          "name price image",
        );
      }
    }
...
app.patch("/api/cart/items", authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    console.log(
      `Looking for item: ${menuItemIdStr}, type: ${itemType}, size: ${selectedSize?.size}`,
    );
    ...
    if (existingItemIndex >= 0) {
      console.log(
        `Found existing item at index ${existingItemIndex}, current quantity: ${cart.items[existingItemIndex].quantity}`,
      );
      ...
      if (quantity <= 0) {
        ...
        console.log(
          `Updating quantity from ${cart.items[existingItemIndex].quantity} to ${quantity}`,
        );
        ...
        if (
          selectedSize &&
          JSON.stringify(cart.items[existingItemIndex].selectedSize) !==
            JSON.stringify(selectedSize)
        ) {
          console.log(
            `Updating size from ${JSON.stringify(cart.items[existingItemIndex].selectedSize)} to ${JSON.stringify(selectedSize)}`,
          );
...
app.delete("/api/cart/items/:itemId", authenticateJWT, async (req, res) => {
  try {
    ...
    cart.items = cart.items.filter(
      (item) =>
        !(item.menuItem.toString() === itemId && item.itemType === itemType),
    );
...
app.post("/api/bookings", async (req, res) => {
  try {
    ...
    if (token) {
      try {
        const decoded = jwt.verify(token, process.env.JWT_SECRET);
        bookingData.userId = decoded.userId;
      } catch (err) {
        // Token invalid, continue as guest booking
      }
    }

    // Store original amount and calculate deposit
    bookingData.originalAmount = bookingData.totalAmount || 0;
...
// Get user's bookings (including guest bookings by email/phone)
...
// Get guest bookings by email and phone
app.post("/api/bookings/guest", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to fetch guest bookings" });
  }
});
...
app.put(
  ...
  async (req, res) => {
    try {
      ...
      console.log(`Price proposed for booking ${booking._id}: ${amount}`);
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to propose price" });
    }
  },
);
...
app.put("/api/bookings/:id/accept-price", async (req, res) => {
  try {
    ...
    if (booking.status !== "price_proposed") {
      return res
        .status(400)
        .json({ success: false, error: "No price proposal to accept" });
    }

    await booking.customerAcceptPrice(message || "Price accepted");
...
// Customer reject proposed price
app.put("/api/bookings/:id/reject-price", async (req, res) => {
  try {
    const { message } = req.body;
    const booking = await Booking.findById(req.params.id);
    ...
    if (booking.status !== "price_proposed") {
      return res
        .status(400)
        .json({ success: false, error: "No price proposal to reject" });
    }

    await booking.customerRejectPrice(message || "Price rejected");

    res.json({ success: true, booking });
  } catch (err) {
    console.error("Error rejecting price:", err);
    res.status(500).json({ success: false, error: "Failed to reject price" });
  }
});
...
app.post("/api/bookings/:id/feedback", async (req, res) => {
  try {
    ...
    // Add admin info if from admin
    if (from === "admin" && req.admin) {
      feedback.adminId = req.admin._id;
      feedback.adminName = req.admin.name;
    }

    await booking.addFeedback(feedback);
...
// Legacy endpoint for payment status update
